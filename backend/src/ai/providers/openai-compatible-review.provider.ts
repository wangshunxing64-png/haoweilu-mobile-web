...
function rateLimitDelayMs(
  ...
  return (
    RATE_LIMIT_RETRY_DELAYS_MS[retryIndex]
    ?? RATE_LIMIT_RETRY_DELAYS_MS.at(-1)
    ?? 0
  );
}
  ...
  implements ReviewProvider
{
  ...
  async generate(
    ...
    if (!this.options.apiKey) {
      throw new Error(
        `${this.options.errorLabel} API key is not configured`,
      );
    }

    const styles = context.merchant.reviewStyles.map(
      ({ id, name, label }) => ({
        styleId: id,
        name,
        label,
    ...
    const systemPrompt = [
      ...
      "鼓励大胆进行主观化、生活化和场景化扩写，可以自然补充用餐氛围、整体感受、聚餐场景、回购意愿以及对服务、环境、性价比的主观判断；但不要自行创造具体发生过的事件，例如赠送菜品、具体吃了几碗、具体打包行为、具体店员动作、具体消费金额、具体等待时间或具体制作时长，除非顾客已经提供。",

      '输出必须是 JSON 对象，格式为 {"reviews":[{"styleId":"...","content":"..."}]}。',

      "必须恰好输出 3 条评价，分别对应给定的三个 styleId，三条在表达角度、节奏和语言风格上要明显不同。",

      `每条评价绝对不得少于 ${MIN_REVIEW_CHARACTERS} 个字符，推荐生成 130 到 ${PREFERRED_MAX_REVIEW_CHARACTERS} 个中文字符。优先保证表达自然、完整、有真人感，可以根据正常餐饮消费场景进行合理扩写，不要为了达到长度要求机械重复或刻意凑字数。`,

      "围绕菜品、体验标签和顾客补充感受展开，但允许自然加入合理的场景感、主观感受和再次消费意愿。",

      "日常分享型要像随手记录；朋友推荐型要体现值得分享和推荐的理由；本地体验型要朴实、具体、有生活感。",

      "语言必须自然、生活化、有个人表达，避免 AI 总结腔、广告宣传腔、官方介绍腔和统一模板腔。",
    ...
    const userPayload = {
      ...
      dishes: context.input.dishes,
      experienceTags: context.input.tags,
      userMessage: context.input.message.trim(),
      styles,
    };
    ...
    const maxRateLimitRetries =
      this.options.maxRateLimitRetries
    ...
    if (!response.ok) {
      throw new Error(
        `${this.options.errorLabel} request failed: ${response.status}`,
      );
    }
    ...
    if (!content) {
      throw new Error(
        `${this.options.errorLabel} response contained no message content`,
      );
    }
    ...
    try {
      parsed =
        JSON.parse(content) as ParsedReviews;
    } catch {
    ...
    if (!Array.isArray(parsed.reviews)) {
      throw new Error(
        `${this.options.errorLabel} response contained no reviews array`,
      );
    }
    ...
    return parsed.reviews.map(
      (review, index) => {
        const style =
          context.merchant.reviewStyles.find(
            (item) => item.id === review.styleId,
          )
